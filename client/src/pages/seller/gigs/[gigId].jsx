...
import { API_URL, EDIT_GIG_DATA, GET_GIG_DATA, IMAGES_URL } from "../../../utils/constants";
...
function EditGig() {
  ...
  useEffect(() => {
    const fetchGigData = async () => {
      try {
        ...
        formik.values.category = gig.category
        formik.values.title = gig.title
        formik.values.price = gig.price
        formik.values.revisions = gig.revisions
        formik.values.shortDesc = gig.shortDesc
        formik.values.subCategory = gig.subCategory
        formik.values.time = gig.deliveryTime
        setDescription(gig.description);
        setfeatures(gig.features);
        const imageFiles = [];
        const fetchImagePromises = gig.images.map((image) => {
  ...
  const editGig = async (value) => {
    try {
      
      const { category,subCategory, price, revisions, time, title, shortDesc } =
        value;
      if(files.length === 0) {
  ...
  return (
    <div className="min-h-[80vh] my-10 mt-0 px-32">
      ...
      <form action="" onSubmit={formik.handleSubmit} className="flex flex-col gap-5 mt-10">
